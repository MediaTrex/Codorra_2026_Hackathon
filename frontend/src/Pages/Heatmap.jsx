            ...
            <div className="relative" style={{ height: 450 }}>
              ...
              {loading ? (
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-slate-500">Loading heatmap data...</span>
                </div>
              ) : heatmapData.length > 0 ? (
                heatmapData.slice(0, 8).map((point, i) => {
                  const color = getColor(point.density_percentage || 0);
                  const radius = 40 + ((point.density_percentage || 0) / 100) * 60;
                  return (
                    <div key={point._id || i} className="absolute group" style={{
                      left: `${15 + (i % 4) * 22}%`,
                      ...
                      <div className="absolute -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/80 text-white text-xs px-2 py-1 rounded-lg whitespace-nowrap z-10">
                        {point.camera_name || `Camera ${i + 1}`}: {Math.round(point.density_percentage || 0)}%
                      </div>
                      <div className="absolute w-2 h-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white" style={{ boxShadow: `0 0 8px ${color}` }}/>
                    </div>
                  );
                })
              ) : (
              ...
              {loading ? (
                ...
                <div className="space-y-3">
                  {cameras.slice(0, 8).map((loc, i) => {
                    const density = loc.density_percentage || loc.capacity || 0;
                    const color = getColor(density);
                    return (
                      <div key={loc._id || i} className="flex items-center gap-3">
                        <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: color, boxShadow: `0 0 8px ${color}` }}/>
                        <span className="text-slate-400 text-xs flex-1 truncate">{loc.name || `Camera ${i + 1}`}</span>
                        <span className="text-xs font-bold" style={{ color }}>{Math.round(density)}%</span>
                      </div>
                    );
                  })}
                </div>
              ) : (
                <div className="text-center text-slate-500 py-4">No camera data available</div>
              )}
            </SectionCard>
          </div>
        </div>
